...
class Builder:
    def __init__(self, options):
        ...
        self.dldt_dir = check_dir(options.dldt_dir)
    ...
    def clean_build_dir(self):
        for d in ["CMakeCache.txt", "CMakeFiles/", "bin/", "lib/", "src/", "thirdparty/", "../bin/"]:
            rm_one(d)
    ...
    def get_build_flags(self):
        ...
        # We are building DLDT libraries as side modules for OpenCV main module.
...
if __name__ == "__main__":
    dldt_dir = os.path.abspath(os.path.join(SCRIPT_DIR, '.'))

    #emscripten_dir = None
    #if "EMSCRIPTEN" in os.environ:
    #    emscripten_dir = os.environ["EMSCRIPTEN"]

    parser = argparse.ArgumentParser(description='Build DLDT by Emscripten')
    parser.add_argument("build_dir", help="Building directory (and output)")
    parser.add_argument('--dldt_dir', default=dldt_dir, help='DLDT source directory (default is "./" relative to script location)')
    ...
    parser.add_argument('--threads', action="store_true", help="Build DLDT with threads optimization")
    ...
    log.info("=====")
    log.info("===== Building DLDT by Emscripten")
    log.info("=====")
    builder.build_dldt()
    ...
    dldt_libs_path = os.path.join(builder.build_dir, "..", "bin", "Release", "lib")
    if check_file(dldt_libs_path):
        log.info("Libs location: %s", dldt_libs_path)
