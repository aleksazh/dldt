...
void jit_avx512_core_x8s8s32x_convolution_fwd_t<src_type,
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_avx512_core_x8s8s32x_convolution.cpp:                  occ1 += jcp.nb_oc_blocking) {" << std::endl;
                ...
                auto bias_w = bias
                    ...
                    : 0;
                int32_t *compensation_w = (jcp.signed_input || jcp.with_input_zp)
                                          ? compensation + g_oc : 0;
...
template <data_type_t src_type, data_type_t dst_type>
void jit_avx512_core_x8s8s32x_convolution_fwd_t<src_type,
        dst_type>::execute_forward_3d() const {
    auto src = reinterpret_cast<const src_data_t *>(this->input_memory(0));
    auto weights = reinterpret_cast<const wei_data_t *>(this->input_memory(1));
    auto bias = reinterpret_cast<const char *>(this->input_memory(2));
    auto dst = reinterpret_cast<dst_data_t *>(this->memory());
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_avx512_core_x8s8s32x_convolution.cpp:                   occ1 += jcp.nb_oc_blocking) {" << std::endl;
                ...
                int i_f_overflow = nstl::min(jcp.kd,
                                             div_up(max(0, -id_s), dilate_d));
                int i_back_overflow = nstl::min(jcp.kd, div_up(
                        max(0, id_s - jcp.id + (jcp.kd - 1) * dilate_d + 1),
                        dilate_d));
                int kd_padding = nstl::max(0,
                                           jcp.kd - i_f_overflow - i_back_overflow);

                size_t wei_d_stride = (!jcp.signed_input && !jcp.with_input_zp)
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_avx512_core_x8s8s32x_convolution.cpp:                  for (int oj = oh_s, ij = ih_s; oj < oh_e; ++oj, ij += jcp.stride_h) {" << std::endl;
                    ...
                    size_t wei_h_stride = (!jcp.signed_input && !jcp.with_input_zp)
                                        ? i_t_overflow * wht_h_stride : 0;
                    p.src = src_w + i_f_overflow * dilate_d * src_d_stride + i_t_overflow * dilate_h * src_h_stride;
                    p.dst = dst_w;
...
template <data_type_t src_type, data_type_t dst_type>
void jit_avx512_core_x8s8s32x_convolution_fwd_t<src_type,
        dst_type>::execute_forward_2d_dw() const {
    auto src = reinterpret_cast<const src_data_t *>(this->input_memory(0));
    auto weights = reinterpret_cast<const wei_data_t *>(this->input_memory(1));
    auto bias = reinterpret_cast<const char *>(this->input_memory(2));
    auto dst = reinterpret_cast<dst_data_t *>(this->memory());
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_avx512_core_x8s8s32x_convolution.cpp:      parallel_nd(jcp.mb, jcp.oh, jcp.nb_ow, nb_groups, [&](int n, int oh_s, int owb, int gg) {" << std::endl;
        ...
        size_t wei_stride = (jcp.signed_input || jcp.with_input_zp) ? 0 : i_t_overflow * wht_h_stride;
        p.src = src_w + i_t_overflow * dilate_h * src_h_stride;
        p.dst = dst_w;
        p.filt = wht_w + wei_stride;
        p.bias = bias_w;
...
template <data_type_t src_type, data_type_t dst_type>
void jit_avx512_core_x8s8s32x_convolution_fwd_t<src_type,
        dst_type>::execute_forward_3d_dw() const {
    auto src = reinterpret_cast<const src_data_t *>(this->input_memory(0));
    auto weights = reinterpret_cast<const wei_data_t *>(this->input_memory(1));
    auto bias = reinterpret_cast<const char *>(this->input_memory(2));
    auto dst = reinterpret_cast<dst_data_t *>(this->memory());
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_avx512_core_x8s8s32x_convolution.cpp:      parallel_nd(jcp.mb, jcp.od, jcp.oh, jcp.nb_ow, nb_groups, [&](int n, int od_s, int oh_s, int owb, int gg) {" << std::endl;
        ...
        auto dst_w = dst + dst_d.blk_off(n, g, od_s, oh_s, ow_s);
