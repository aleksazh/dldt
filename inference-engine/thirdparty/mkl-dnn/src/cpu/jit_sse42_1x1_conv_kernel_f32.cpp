...
{
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:  void jit_sse42_1x1_conv_kernel_f32::generate_bcast_loop(int load_loop_blk) {" << std::endl;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:          for (int i = 0; i < num_substeps; i++) {" << std::endl;
            generate_reduce_loop(load_loop_blk, jcp.ur);
            if (i < num_substeps - 1) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:              if (i < num_substeps - 1) {" << std::endl;
    ...
    L(bcast_loop_tail);
    if (jcp.ur_tail) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      if (jcp.ur_tail) {" << std::endl;
...
{
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:          int load_loop_blk, int ur) {" << std::endl;
    auto reg_load = [=](int i, int n) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto reg_load = [=](int i, int n) {" << std::endl;
        return Xmm(2*ur * load_loop_blk + 2*i + n + 1);
    };

    auto reg_accum = [=](int i, int j, int n) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto reg_accum = [=](int i, int j, int n) {" << std::endl;
        return Xmm(n*load_loop_blk*ur + i*ur + j + 1);
    };

    auto bias_ptr = [=](int i, int n) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto bias_ptr = [=](int i, int n) {" << std::endl;
        return ptr[reg_bias_data + sizeof(float) * jcp.oc_block * i + n*4*sizeof(float)];
    };

    auto bcast_ptr = [=](int u, int j) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto bcast_ptr = [=](int u, int j) {" << std::endl;
        assert(j < jcp.ur);
        assert(u <= jcp.reduce_loop_unroll);
        size_t offt;
        if (one_of(jcp.prop_kind,
                    forward_training, forward_inference, backward_data)) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:                      forward_training, forward_inference, backward_data)) {" << std::endl;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto load_ptr = [=](int u, int i, int n) {" << std::endl;
        size_t offt;
        size_t u0 = u % jcp.reduce_loop_unroll;
        size_t u1 = u / jcp.reduce_loop_unroll;
        switch (jcp.prop_kind) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:          switch (jcp.prop_kind) {" << std::endl;
        case backward_data:
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto output_ptr = [=](int i, int j, int n) {" << std::endl;
        switch (jcp.prop_kind) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:          switch (jcp.prop_kind) {" << std::endl;
        case backward_data:
    ...
    auto init = [=]() {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto init = [=]() {" << std::endl;
        Label init_done;
        Label init_zero;

        if (jcp.with_bias && one_of(jcp.prop_kind, forward_training,
                    forward_inference)) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:                      forward_inference)) {" << std::endl;
            test(reg_reduce_pos_flag, FLAG_REDUCE_FIRST);
            jz(init_zero);

            for (int i = 0; i < load_loop_blk; i++)
                for (int j = 0; j < ur; ++j) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:                  for (int j = 0; j < ur; ++j) {" << std::endl;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:              for (int j = 0; j < ur; ++j) {" << std::endl;
        ...
        // load weights
        for (int i = 0; i < load_loop_blk; ++i) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:          for (int i = 0; i < load_loop_blk; ++i) {" << std::endl;
    ...
    auto store = [=]() {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto store = [=]() {" << std::endl;
        Label store_noadd;

        if (!jcp.with_sum) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:          if (!jcp.with_sum) {" << std::endl;
            test(reg_reduce_pos_flag, FLAG_REDUCE_FIRST);
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:          for (int i = 0; i < end_idx; i++) {" << std::endl;
            auto& post_op = p.entry_[i];
            if (post_op.is_eltwise()) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:              if (post_op.is_eltwise()) {" << std::endl;
                eltwise_injectors[eltwise_inj_idx]->compute_vector_range(1, 2 * ur * load_loop_blk + 1);
                eltwise_inj_idx++;
            } else if (post_op.is_depthwise()) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:              } else if (post_op.is_depthwise()) {" << std::endl;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:                  for (int j = 0; j < load_loop_blk; ++j) {" << std::endl;
                    for (int k = 0; k < 2; k++) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:                      for (int k = 0; k < 2; k++) {" << std::endl;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:              } else if (post_op.is_quantization()) {" << std::endl;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:                      for (int k = 0; k < 2; k++) {" << std::endl;
                        ...
                        for (int u = 0; u < ur; u++) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:                          for (int u = 0; u < ur; u++) {" << std::endl;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto fma_block = [=](bool last_block) {" << std::endl;
        for (int u = 0; u < jcp.reduce_loop_unroll; ++u) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:          for (int u = 0; u < jcp.reduce_loop_unroll; ++u) {" << std::endl;
            for (int j = 0; j < ur; ++j) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:              for (int j = 0; j < ur; ++j) {" << std::endl;
                for (int i = 0; i < load_loop_blk; ++i) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:                  for (int i = 0; i < load_loop_blk; ++i) {" << std::endl;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:                      if (j == ur - 1 && !(last_block && u == jcp.reduce_loop_unroll - 1)) {" << std::endl;
                        movups(reg_load(i, 0), load_ptr(u + 1, i, 0));
                        movups(reg_load(i, 1), load_ptr(u + 1, i, 1));
                    }
                }
                if (j < ur - 1) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:                  if (j < ur - 1) {" << std::endl;
                    movss(reg_bcast, bcast_ptr(u, j + 1));
                    shufps(reg_bcast, reg_bcast, 0);
                }
            } // for ur
            if (!last_block || u < jcp.reduce_loop_unroll - 1) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:              if (!last_block || u < jcp.reduce_loop_unroll - 1) {" << std::endl;
...
{
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:  void jit_sse42_1x1_conv_kernel_f32::generate_diff_bias_loop(int load_loop_blk) {" << std::endl;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto diff_bias_ptr = [=](int i, int n) {" << std::endl;
    ...
    auto diff_bias_reg = [=](int i, int n) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto diff_bias_reg = [=](int i, int n) {" << std::endl; return Xmm(2*i + n + 1); };
...
{
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:  void jit_sse42_1x1_conv_kernel_f32::generate() {" << std::endl;
    const auto &p = attr_.post_ops_;
    int end_idx = jcp.with_dw_conv ? p.find(primitive_kind::convolution) : p.len_;
    for (int i = 0; i < end_idx; i++) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      for (int i = 0; i < end_idx; i++) {" << std::endl;
    ...
    if (jcp.with_bias) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      if (jcp.with_bias) {" << std::endl;
        if (jcp.prop_kind == backward_weights) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:          if (jcp.prop_kind == backward_weights) {" << std::endl;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto generate_load_loop_body = [=] (int load_loop_blk) {" << std::endl;
...
bool jit_sse42_1x1_conv_kernel_f32::post_ops_ok(
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:          jit_1x1_conv_conf_t &jcp, const primitive_attr_t &attr) {" << std::endl;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto all_post_ops_supported = [&]() {" << std::endl;
        bool ok = true;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto contain = [&](mkldnn::impl::primitive_kind_t kind) {" << std::endl; return p.find(kind, 0, dw_conv_idx) != -1; };
    auto position = [&](mkldnn::impl::primitive_kind_t kind) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto position = [&](mkldnn::impl::primitive_kind_t kind) {" << std::endl; return p.find(kind, 0, dw_conv_idx); };
    auto count = [&](mkldnn::impl::primitive_kind_t kind) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      auto count = [&](mkldnn::impl::primitive_kind_t kind) {" << std::endl; return p.count(kind, 0, dw_conv_idx); };
...
{
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:          const primitive_attr_t &attr) {" << std::endl;
    if (!mayiuse(sse42))
    ...
    if (jcp.with_dw_conv) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      if (jcp.with_dw_conv) {" << std::endl;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      if (one_of(jcp.prop_kind, forward_training, forward_inference)) {" << std::endl;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      } else if (jcp.prop_kind == backward_data) {" << std::endl;
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:      } else if (jcp.prop_kind == backward_weights) {" << std::endl;
        ...
        while (true) {
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:          while (true) {" << std::endl;
...
void jit_sse42_1x1_conv_kernel_f32::init_scratchpad(
    ...
    std::cerr << "./inference-engine/thirdparty/mkl-dnn/src/cpu/jit_sse42_1x1_conv_kernel_f32.cpp:          const jit_1x1_conv_conf_t &jcp, const jit_conv_conf_t &jcp_dw) {" << std::endl;
