...
public:
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:      explicit ExperimentalSparseWeightedReduceImpl(const CNNLayer* layer) {" << std::endl;
        try {
            if ((layer->insData.size() != 5 && layer->insData.size() != 6) || layer->outData.size() != 1) {
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              if ((layer->insData.size() != 5 && layer->insData.size() != 6) || layer->outData.size() != 1) {" << std::endl;
                THROW_IE_EXCEPTION << layer->name << " Incorrect number of input/output edges!";
            }
            if (layer->insData.size() == 6) {
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              if (layer->insData.size() == 6) {" << std::endl;
                with_weights = true;
            }
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              if (are_other_precisions_valid == false) {" << std::endl;
                THROW_IE_EXCEPTION << layer->name << " Incorrect precision of the input tensors.";
            }

            if (input_parameters_table_precision != Precision::FP32) {
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              if (input_parameters_table_precision != Precision::FP32) {" << std::endl;
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              if (input_default_value_precision != Precision::I32) {" << std::endl;
                THROW_IE_EXCEPTION << layer->name
                                   << " Incorrect precision of the input default value. Only I32 is supported!";
            }

            if (with_weights) {
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              if (with_weights) {" << std::endl;
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:                  if (input_weights_precision != Precision::FP32) {" << std::endl;
                    THROW_IE_EXCEPTION << layer->name
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              if (input_indices_dims.size() != 2 || input_indices_dims[1] != 2) {" << std::endl;
                THROW_IE_EXCEPTION << layer->name
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              if (input_values_dims.size() != 1) {" << std::endl;
                THROW_IE_EXCEPTION << layer->name
                                   << " Incorrect dimensions for input values. It must be N dimension tensor.";
            }
            if (input_indices_dims[0] != input_values_dims[0]) {
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              if (input_indices_dims[0] != input_values_dims[0]) {" << std::endl;
                THROW_IE_EXCEPTION << layer->name
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              if (input_dense_shape_dims.size() != 1 || input_dense_shape_dims[0] != 2) {" << std::endl;
                THROW_IE_EXCEPTION << layer->name
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              if (input_parameters_table_dims.size() < 2) {" << std::endl;
                THROW_IE_EXCEPTION << layer->name
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              if (input_default_value_dims.size() != 0) {" << std::endl;
                THROW_IE_EXCEPTION << layer->name
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:                  if (input_weights_dims.size() != 1) {" << std::endl;
                    THROW_IE_EXCEPTION << layer->name
                                       << " Incorrect dimensions for input weights. It must be N dimension tensor.";
                }
                if (input_weights_dims[0] != input_values_dims[0]) {
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:                  if (input_weights_dims[0] != input_values_dims[0]) {" << std::endl;
                    THROW_IE_EXCEPTION << layer->name
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              if (output_dims.size() != input_parameters_table_dims.size()) {" << std::endl;
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              for (size_t ind = 1; ind < input_parameters_table_dims.size(); ind++) {" << std::endl;
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:          catch (InferenceEngine::details::InferenceEngineException &ex) {" << std::endl;
            errorMsg = ex.what();
        }
    }
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:          for (size_t batch_ind = 0; batch_ind < output_batch_size; batch_ind++) {" << std::endl;
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              for (size_t ind = 0; ind < output_elem_size; ind++) {" << std::endl;
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:          for (size_t curr_value_ind = 0; curr_value_ind < input_num_values; curr_value_ind++) {" << std::endl;
            int indice_x = 0;
            size_t value = 0;
    ...
    std::cerr << "./inference-engine/src/mkldnn_plugin/nodes/sparse_weighted_reduce.cpp:              if (prev_indice_x != indice_x) {" << std::endl;
                // zero a slice
