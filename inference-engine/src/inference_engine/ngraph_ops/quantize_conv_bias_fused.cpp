...
constexpr NodeTypeInfo op::QuantizedConvolutionBiasFused::type_info;

op::QuantizedConvolutionBiasFused::QuantizedConvolutionBiasFused(
    const std::shared_ptr<ngraph::op::QuantizedConvolutionBias>& qconv, const std::shared_ptr<ngraph::Node>& w_scale)
...
op::QuantizedConvolutionBiasFused::QuantizedConvolutionBiasFused(
    ...
    const Strides& window_movement_strides, const Strides& window_dilation_strides, const CoordinateDiff& padding_below,
    const CoordinateDiff& padding_above, const Strides& data_dilation_strides, const std::shared_ptr<Node> scale,
    const std::shared_ptr<Node> w_scale, const bool with_relu)
    ...
    set_output_type(0, output_et,
                    util::infer_convolution_output_shape(this, data_batch_shape, filters_shape, window_movement_strides,
                                                         window_dilation_strides, padding_below, padding_above,
                                                         data_dilation_strides, 0, /* batch_axis_data,              */
...
shared_ptr<Node> op::QuantizedConvolutionBiasFused::copy_with_new_args(const NodeVector& new_args) const {
    ...
    return shared_ptr<Node>(
        new QuantizedConvolutionBiasFused(new_args.at(0), new_args.at(1), new_args.at(2), get_window_movement_strides(),
                                          get_window_dilation_strides(), get_padding_below(), get_padding_above(),
                                          get_data_dilation_strides(), new_args.at(3), new_args.at(4), m_with_relu));
}