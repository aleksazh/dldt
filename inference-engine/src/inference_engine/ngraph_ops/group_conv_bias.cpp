...
op::GroupConvolutionBias::GroupConvolutionBias(const shared_ptr<op::GroupConvolution>& conv,
                                               const shared_ptr<Node>& bias, const bool with_relu)
    : Op("GroupConvolutionBias", check_single_output_args({conv->get_argument(0), conv->get_argument(1), bias})),
    ...
    validate_groupconvbias_shapes(conv->get_argument(0)->get_shape(), conv->get_argument(1)->get_shape(),
                                  bias->get_shape(), conv->get_output_shape(0), conv->get_groups());

    set_output_type(0, conv->get_element_type(), conv->get_output_shape(0));
}

op::GroupConvolutionBias::GroupConvolutionBias(const shared_ptr<Node>& data_batch, const shared_ptr<Node>& filters,
                                               const shared_ptr<Node>& bias, const Strides& window_movement_strides,
                                               const Strides& window_dilation_strides,
                                               const CoordinateDiff& padding_below, const CoordinateDiff& padding_above,
                                               const Strides& data_dilation_strides, size_t groups,
                                               const Shape& output_shape, bool with_relu, float alpha)
    ...
    validate_groupconvbias_shapes(data_batch_shape, filters_shape, bias->get_shape(), output_shape, groups);
...
shared_ptr<Node> op::GroupConvolutionBias::copy_with_new_args(const NodeVector& new_args) const {
    ...
    return shared_ptr<Node>(new GroupConvolutionBias(
        new_args.at(0), new_args.at(1), new_args.at(2), get_window_movement_strides(), get_window_dilation_strides(),
        get_padding_below(), get_padding_above(), get_data_dilation_strides(), get_groups(), get_output_shape(0),
        m_with_relu, get_alpha()));
}
