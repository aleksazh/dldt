...
op::Eltwise::Eltwise(const Output<Node>& data1, const Output<Node>& data2, const ELTWISE_TYPE eltwise_type)
    ...
    std::cerr << "./inference-engine/src/inference_engine/ngraph_ops/eltwise.cpp:      : Op({data1, data2}), eltwise_type(eltwise_type) {" << std::endl;
    constructor_validate_and_infer_types();
}

std::shared_ptr<Node> op::Eltwise::copy_with_new_args(const NodeVector& new_args) const {
    if (new_args.size() != 2) {
    std::cerr << "./inference-engine/src/inference_engine/ngraph_ops/eltwise.cpp:      if (new_args.size() != 2) {" << std::endl;
...
void op::Eltwise::validate_and_infer_types() {
    std::cerr << "./inference-engine/src/inference_engine/ngraph_ops/eltwise.cpp:  void op::Eltwise::validate_and_infer_types() {" << std::endl;
    ...
    std::cerr << "./inference-engine/src/inference_engine/ngraph_ops/eltwise.cpp:      while (shape1_it != shape1.rend() || shape2_it != shape2.rend()) {" << std::endl;
        if (shape1_it != shape1.rend() && shape2_it != shape2.rend()) {
    std::cerr << "./inference-engine/src/inference_engine/ngraph_ops/eltwise.cpp:          if (shape1_it != shape1.rend() && shape2_it != shape2.rend()) {" << std::endl;
            *output_shape_it = std::max(*shape1_it, *shape2_it);
        } else if (shape1_it != shape1.rend()) {
    std::cerr << "./inference-engine/src/inference_engine/ngraph_ops/eltwise.cpp:          } else if (shape1_it != shape1.rend()) {" << std::endl;
            *output_shape_it = *shape1_it;
        } else if (shape2_it != shape2.rend()) {
    std::cerr << "./inference-engine/src/inference_engine/ngraph_ops/eltwise.cpp:          } else if (shape2_it != shape2.rend()) {" << std::endl;
    ...
    std::cerr << "./inference-engine/src/inference_engine/ngraph_ops/eltwise.cpp:          if (output_shape_it == output_shape.rend()) {" << std::endl;
            break;
        }
    }
