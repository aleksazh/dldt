...
bool get_single_value(const std::shared_ptr<op::Constant>& const_node, float& value) {
    std::cerr << "./inference-engine/src/inference_engine/transform/transformations/utils/utils.cpp:  bool get_single_value(const std::shared_ptr<op::Constant>& const_node, float& value) {" << std::endl;
    switch (const_node->get_element_type()) {
    std::cerr << "./inference-engine/src/inference_engine/transform/transformations/utils/utils.cpp:      switch (const_node->get_element_type()) {" << std::endl;
...
std::shared_ptr<Node> normalize_constant(const std::shared_ptr<op::Constant>& constant,
                                         const Shape& shape) {
    std::cerr << "./inference-engine/src/inference_engine/transform/transformations/utils/utils.cpp:                                           const Shape& shape) {" << std::endl;
    auto const_shape = constant->get_shape();
    if (const_shape.size() == shape.size()) {
    std::cerr << "./inference-engine/src/inference_engine/transform/transformations/utils/utils.cpp:      if (const_shape.size() == shape.size()) {" << std::endl;
        return constant;
    }
    int cnt = shape.size() - const_shape.size();
    for (int i = 0; i < cnt; ++i) {
    std::cerr << "./inference-engine/src/inference_engine/transform/transformations/utils/utils.cpp:      for (int i = 0; i < cnt; ++i) {" << std::endl;
...
std::shared_ptr<Node> broadcastTo(const Output<Node>& input, const ngraph::Shape& shape) {
    std::cerr << "./inference-engine/src/inference_engine/transform/transformations/utils/utils.cpp:  std::shared_ptr<Node> broadcastTo(const Output<Node>& input, const ngraph::Shape& shape) {" << std::endl;
    IE_ASSERT(input.get_shape().size() == shape.size());
    for (size_t i = 0; i < input.get_shape().size(); i++) {
    std::cerr << "./inference-engine/src/inference_engine/transform/transformations/utils/utils.cpp:      for (size_t i = 0; i < input.get_shape().size(); i++) {" << std::endl;
...
std::shared_ptr<ngraph::Node> reshapeTo(const Output<Node> & input, const Shape& shape) {
    std::cerr << "./inference-engine/src/inference_engine/transform/transformations/utils/utils.cpp:  std::shared_ptr<ngraph::Node> reshapeTo(const Output<Node> & input, const Shape& shape) {" << std::endl;
...
bool constantIsEqualTo(const std::shared_ptr<ngraph::op::Constant>& const_node, float value, float eps) {
    std::cerr << "./inference-engine/src/inference_engine/transform/transformations/utils/utils.cpp:  bool constantIsEqualTo(const std::shared_ptr<ngraph::op::Constant>& const_node, float value, float eps) {" << std::endl;
    float res(0);
    if (!get_single_value(const_node, res)) {
    std::cerr << "./inference-engine/src/inference_engine/transform/transformations/utils/utils.cpp:      if (!get_single_value(const_node, res)) {" << std::endl;
        return false;
    }
